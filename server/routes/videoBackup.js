...
router.post("/merge-videos", (req, res) => {
  ...
  const gloss = convertToGloss(englishSentence);

  // Send gloss text as a response
  res.json({ gloss });
});
...
router.post("/get-video", (req, res) => {
  ...
  if (!glosses || glosses.length === 0) {
    return res.status(400).send("No glosses provided");
  }
  ...
  const simplifiedWords = contentWords
    .map((word) => {
      if (temporalWordList.includes(word)) {
        ...
        return word.replace(/(ed)$/, ""); // rudimentary verb simplification
      } else {
        return word;
      }
    })
